<script lang="ts">
  ...
  onMount(async () => {
    ...
    const [experienceSnapshot, experienceError] = await trycatch(getDocs(collection($db, "experience")));
    const [skillsSnapshot, skillsError] = await trycatch(getDocs(collection($db, "skills")));

    if (!socialsError) socials.set(socialsSnapshot.map((social) => social.data()));
    if (!projectsError) projects.set(projectsSnapshot.map((project) => project.data()));
    if (!experienceError) experience.set(experienceSnapshot.map((job) => job.data()));
    if (!skillsError) skills.set(skillsSnapshot.map((skill) => skill.data()));
  })
</script>
